...
select product_location_ID, product_price.location_ID, cross_street, city, province, date_opened
from product_price
left join location_info on product_price.location_ID = location_info.location_ID;

--in case you want to see all the details at once (product info and location info together)
select * from product_info
full join product_price on product_info.product_ID = product_price.product_ID
full join location_info on product_price.location_ID = location_info.location_ID;
...
select location_info.cross_street, avg(price) from product_price
left join location_info on product_price.location_ID = location_info.location_ID
